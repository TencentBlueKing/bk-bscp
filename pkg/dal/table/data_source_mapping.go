...
func (v Columns_) ValidateColumn(kit *kit.Kit, re *regexp.Regexp) error {
	...
	// 主键必须唯一且不能为空
	if v.Primary {
		if !v.Unique && v.NotNull {
			return errf.Errorf(errf.InvalidArgument,
				i18n.T(kit, "the primary key must be unique and cannot be empty"))
		}
	}
	// 自增的字段不能为空
	...
	if err := v.validateDefaultValue(kit, v.DefaultValue); err != nil {
		return err
	}
...
// validateDefaultValue 验证字段默认值
func (c Columns_) validateDefaultValue(kit *kit.Kit, value string) error {
	if value == "" {
		return nil
	}
	switch c.ColumnType {
	case JsonColumn:
		...
		if err := json.Unmarshal([]byte(value), &jsonValue); err != nil {
			return errors.New(i18n.T(kit, "value must be valid JSON: %v", err))
		}
	case NumberColumn:
		// 尝试将字符串解析为数字
		if _, err := strconv.ParseFloat(value, 64); err != nil {
			return errors.New(i18n.T(kit, "value must be a valid number"))
		}
	}
