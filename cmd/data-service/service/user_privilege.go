...
// 获取配置项权限列表
func (s *Service) listConfigItemPerms(kit *kit.Kit, bizID, appID uint32,
	data []*table.ConfigItem) ([]*table.ConfigItem, error) {

	for _, v := range data {
		permissions, err := s.getConfigItemPerms(kit, bizID, appID, v)
		if err != nil {
			return nil, err
		}
...
// 获取单个配置项权限
func (s *Service) getConfigItemPerms(kit *kit.Kit, bizID, appID uint32,
	data *table.ConfigItem) (*table.ConfigItem, error) {
...
func (s *Service) handleUserPrivilege(kit *kit.Kit, tx *gen.QueryTx, bizID, appID, templateSpaceID uint32,
	...
	if (data.Spec.User == "" || data.Spec.User == "root") && data.Attachment.Uid == 0 {
		return nil
	}
	...
	existingUID, err := s.dao.UserPrivilege().GetUserPrivilegeByUid(kit, tx, bizID, appID, templateSpaceID,
		data.Attachment.Uid)
...
func (s *Service) getTplRevPerm(kit *kit.Kit, bizID uint32,
	...
	userPrivilegesMap, userGroupPrivilegesMap := map[string]uint32{}, map[string]uint32{}
	for _, v := range userPrivileges {
		userPrivilegesMap[v.User] = v.Uid
	}
	...
	tmplRevisions.Spec.Permission.Uid = userPrivilegesMap[tmplRevisions.Spec.Permission.User]
	tmplRevisions.Spec.Permission.Gid = userPrivilegesMap[tmplRevisions.Spec.Permission.UserGroup]
