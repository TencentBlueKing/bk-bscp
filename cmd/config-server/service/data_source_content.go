...
// UpdateTableContent implements pbcs.ConfigServer.
func (s *Service) UpdateTableContent(ctx context.Context, req *pbcs.UpdateTableContentReq) (
	*pbcs.UpdateTableContentResp, error) {
	...
	contents := make([]*pbds.UpdateTableContentReq_Item, 0)
	for _, v := range req.GetContents() {
		contents = append(contents, &pbds.UpdateTableContentReq_Item{
			TableContentId: v.TableContentId,
			Content:        v.GetContent(),
		})
	}

	resp, err := s.client.DS.UpdateTableContent(kit.RpcCtx(), &pbds.UpdateTableContentReq{
		BizId:               req.BizId,
		DataSourceMappingId: req.DataSourceMappingId,
		Contents:            contents,
	})
