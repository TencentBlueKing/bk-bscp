export default {
  ...
  模板与数据源: 'Templates and Data Sources',
  ...
  托管表格配置: 'Managed table config',
  关联外部数据源: 'Associate external data source',
  表格配置: 'Table config',
  新建托管表格: 'New managed table',
  请选择表格: 'Please select table',
  ...
  // 数据源管理
  表格数据源管理: 'Table data source management',
  托管表格: 'Managed table',
  外部数据源: 'External data source',
  新建表格: 'Create table',
  '表格名称/表格描述/最近更新人': 'Table name/Table description/Last updated by',
  表格名称: 'Table name',
  表格描述: 'Table description',
  关联配置项: 'Associated configuration item',
  最近更新人: 'Last updated by',
  最近更新时间: 'Last updated time',
  编辑数据: 'Edit data',
  编辑表结构: 'Edit table structure',
  导入表格: 'Import table',
  导出表格: 'Export table',
  创建并编辑数据: 'Create and edit data',
  字段设置: 'Field settings',
  添加字段: 'Add field',
  请先上传文件: 'Please upload the file first',
  基本信息: 'Basic information',
  可见范围: 'Visible range',
  请先设置枚举值: 'Please set the enumeration value first',
  设置枚举值: 'Set enumeration value',
  单选: 'Single selection',
  多选: 'Multiple selection',
  显示文本: 'Display text',
  实际值: 'Actual value',
  字段名: 'Field name',
  '字段名包含字母、数字、下划线 ( _ ) 和美元符号 ( $ )，长度不超过 64 字符': 'The field name contains letters, numbers, underscores (_), and dollar signs ($), and is no more than 64 characters long',
  显示名: 'Display name',
  '默认值/枚举值': 'Default value/Enumeration value',
  主键: 'Primary key',
  非空: 'Non-empty',
  唯一: 'Unique',
  自增: 'Auto-increment',
  ...
  文件来源: 'File source',
  选择文件: 'Select file',
  '支持 .xlsx / .xls / .csv / .sql 文件，后台会自动检测文件类型，配置项格式请参照': 'Support .xlsx / .xls / .csv / .sql files. The background will automatically detect the file type. For the configuration item format, please refer to',
  表结构来源: 'Table structure source',
  手动创建表结构: 'Manually create table structure',
  '目前没有表格结构及数据信息，需要先手动创建表结构，然后手动录入数据': 'Currently, there is no table structure and data information. You need to manually create the table structure first, and then manually enter the data',
  从本地文件导入: '从本地文件导入',
  '可以从本地导入 Excel/CSV 格式的数据文件（.xlsx/.xls/.csv)，还可以从带有 .sql 后缀的 MySQL dump 文件中导入表结构与数据': 'You can import Excel/CSV format data files (.xlsx/.xls/.csv) from your local computer, or import table structure and data from MySQL dump files with the .sql suffix',
  工作表: 'Sheet',
  正在匹配表格字段: 'Matching table fields',
  '表格字段匹配，可继续导入': 'Table field matching, you can continue to import',
  '表格字段有差异，请先确认调整': 'There are differences in the table fields, please confirm and adjust first',
  导入前清空原有数据: 'Clear the original data before importing',
  '主键校验：': 'Primary key validation：',
  '当导入表的主键值出现重合，将直接': 'When the primary key values of the imported table overlap, they will be directly',
  使用导入表的数据覆盖: 'Use the data of the imported table to overwrite',
  '字段校验：': 'Field validation：',
  '导入表删除字段后将更新表结构，该字段': 'After deleting the field of the imported table, the table structure will be updated, and the field',
  已有数据也将同时被清空: 'The existing data will also be cleared at the same time',
  批量设置字段值: 'Batch set field values',
  常用查询: 'Common query',
  基础查询: 'Basic query',
  高级查询: 'Advanced query',
  导出: '导出',
  '可设置字段默认值；ENUM 类型请设置枚举值': 'You can set the default value of the field; for ENUM types, please set the enumeration value',
  请先添加字段: 'Please add a field first',
  新建表格成功: 'Create table successfully',
  编辑表格成功: 'Edit table successfully',
