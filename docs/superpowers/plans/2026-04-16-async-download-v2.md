...
**架构：** 在 gRPC 边界保持 `AsyncDownload` 和 `AsyncDownloadStatus` 不变。内部新增 V2 batch/task 的 Redis 记录、按 `(fileVersionKey, targetID)` 维度的轻量 inflight 去重项、由 due queue 驱动并带有 dispatch lease/repair 的 scheduler，以及一条迁移路径：首个 V2 版本只创建 V2 task，但仍继续消费历史 V1 job。
...
- `cmd/feed-server/bll/asyncdownload/v2_repair.go`
  - orphan task、陈旧 inflight key、终态 batch 清理的 repair/fan-out 辅助逻辑。
...
| `dispatchHeartbeatSeconds` | `15` | 秒 | 分发中的批次用于续租/心跳的时间粒度配置，用来配合分发租约判断批次是否仍然存活。 |
| `dispatchLeaseSeconds` | `60` | 秒 | 分发租约时长。超过这个时间仍未正常推进的批次，会被视为需要 repair 的异常分发。 |
...
| `bscp_async_download_v2_batch_state_count` | Counter | `biz`, `app`, `file`, `state` | V2 batch 进入某个状态的次数。当前会覆盖 `Collecting`、`Dispatching`、`Done`、`Partial`、`Failed`。 |
| `bscp_async_download_v2_batch_state_duration_seconds` | Histogram | `biz`, `app`, `file`, `state` | V2 batch 在某个状态停留的耗时分布。当前重点是 `Collecting` 和 `Dispatching` 两段。 |

### 2. 任务推进与结果收敛

| 指标名 | 类型 | 标签 | 说明 |
| --- | --- | --- | --- |
| `bscp_async_download_v2_task_state_count` | Counter | `biz`, `app`, `file`, `state` | V2 task 进入某个状态的次数。当前会覆盖 `Pending`、`Running`、`Success`、`Failed`、`Timeout`。 |
| `bscp_async_download_v2_task_state_duration_seconds` | Histogram | `biz`, `app`, `file`, `state` | V2 task 在某个状态停留的耗时分布。用于看任务从排队到运行、从运行到最终结果分别花了多久。 |
...
| 指标名 | 类型 | 标签 | 说明 |
| --- | --- | --- | --- |
| `bscp_async_download_task_repair_count` | Counter | `reason` | repair 将异常悬挂任务补偿为失败的次数。当前原因包括 `batch_failed`、`orphan_after_dispatch_cutoff`。 |
