...
`biz_id` 继续作为业务、CMDB、租户反查、权限兼容的主边界。项目用于应用配置工作区隔离，环境只用于应用配置数据、版本和发布生命周期隔离。
...
9. **可观测可回滚**：新能力通过灰度开关控制，回填任务可暂停、恢复、重跑；失败时关闭新入口，旧路径继续工作，DDL 不作为回滚前提。
...
服务列表仍然是“项目下的服务清单”。选择某个环境后，表格展示该服务在当前环境下的运营态：
...
| 表 | 目标作用域 | 改造方式 |
| --- | --- | --- |
| `applications` | ProjectScope | 新增 `project_id` |
| `archived_apps` | ProjectScope | 新增 `project_id` |
| `groups` | ProjectScope | 新增 `project_id` |
| `group_app_binds` | ProjectScope | 新增 `project_id`，校验 group/app 属于同一项目 |
...
迁移分为结构迁移、默认 scope 初始化和数据回填三类。

结构迁移继续使用现有 data-service `migrate up` 机制，不在服务启动时执行大表 DDL。
...
```text
1. 访问某个 biz 时，如果默认 project/env 不存在，轻量创建
2. 后台任务补齐历史 biz 的默认 project/env
3. 不在启动阶段全量扫描所有 biz
```
...
每批提交，记录 `last_id`，失败后可重试。回填任务必须幂等，多实例部署时使用分布式锁或唯一索引避免重复执行。
...
旧接口中带 `app_id` 的请求必须额外校验 app 属于 default project，不能只用 `biz_id + app_id` 查询，避免旧接口访问到非默认项目的数据。
...
/api/v1/config/biz/{biz_id}/projects/{project_id}/apps
...
老客户端传 app_id -> 校验 app_id 属于 default project
老客户端只传 app name -> 只在 default project 内按 app name 查找
...
如果未来允许不同项目下存在同名 app，老客户端无法区分项目。兼容边界应明确为：
...
```text
旧 app 权限 -> 映射到 default project 下的 app 权限
...
上线目标是用户基本无感：旧 UI、旧接口、旧客户端持续可用，新项目和新环境能力通过灰度开关逐步开放。

```text
1. 发布只包含表结构扩展的版本
2. 启用 default project/env 懒创建
3. 开启旧接口默认 scope 解析
4. 启动后台回填
...
8. 替换唯一索引
9. 新 UI/API/SDK 支持显式 project/env
10. 灰度开启新项目和新环境能力
11. 字段收紧为 NOT NULL
...
旧 SDK 拉取结果不变
默认项目/默认环境数据完整
旧接口不能访问非 default project 的 app 数据
进程配置管理接口不要求 project/env
同一项目下多个环境复用同一套模板资产
新项目之间 app/group/template 名称互不冲突
同一 app 在不同环境可发布不同 release
...
合理方案不是把所有资源强行挂到环境下，而是保留业务级能力，新增项目级应用配置工作区，并让环境只作用于应用配置数据、版本、发布和客户端状态。

结构迁移走现有 `migrate up`，启动时只做轻量检查和懒创建能力，不做大表 DDL、不做全量回填。旧接口、旧客户端、旧 feed 协议全部默认落到默认项目和默认环境，从而保证现有业务连续可用。
